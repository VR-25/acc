...
-s|--set <resume-stop preset>   可以为 4041|endurance+, 5960|endurance, 7580|default, 8090|lite, 9095|travel
  例如,
    acc -s endurance+ (换言之, "锂电池最佳工作状态"; 最适合 GPS 导航和其他长时间操作)
    acc -s travel (当你需要榨干电池时)
    acc -s 7580 (恢复默认电量设置 (5,60,75-80))
...
提示
  ...
  最后一个命令可以用于在睡觉前编排充电。在这种情况下，后台程序必须处于运行状态.
    例如, acc 45 44 && acc --set applyOnPlug usb/current_max:500000 && sleep $((60*60*7)) && acc 80 75 && acc --set applyOnPlug usb/current_max:2000000
