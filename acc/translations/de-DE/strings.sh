...
print_already_running() {
  echo "(i) accd läuft bereits"
}

print_started() {
  echo "(i) accd gestartet"
}

print_stopped() {
  echo "(i) accd gestoppt"
}

print_not_running() {
  echo "(i) accd läuft nicht"
}

print_restarted() {
  echo "(i) accd neugestartet"
}

print_is_running() {
  echo "(i) accd $1 läuft $2"
}
...
print_charging_disabled_until() {
  echo "(i) Ladevorgang deaktiviert, bis die Batteriekapazität >= $1 entspricht"
}

print_charging_disabled_for() {
  echo "(i) Laden deaktiviert für $1"
}

print_charging_disabled() {
  echo "(i) Laden deaktiviert"
}

print_charging_enabled_until() {
  echo "(i) Ladevorgang aktiviert, bis die Batteriekapazität >= $1 entspricht"
}

print_charging_enabled_for() {
  echo "(i) Laden aktiviert für $1"
}

print_charging_enabled() {
  echo "(i) Laden aktiviert"
}
...
z.B.,
    ...
    -s|--set v|--voltage [millivolts|-] [--exit] Maximale Ladespannung setzen/drucken/wiederherstellen (Bereich: 3700-4200$(print_mV))
        z.B.,
            ...
            acc -s v 3920 --exit (stoppt den Daemon nach Übernahme der Einstellungen)
    ...
    -T|--logtail Überwachung des accd-Protokolls (tail -F)
        z.B., acc -T
    -u|--upgrade [-c|--changelog] [-f|--force] [-k|--insecure] [-n|--non-interactive] Online-Upgrade/Downgrade (erfordert curl)
z.B.,
...
Tipps:
    ...
    Beispielprofil
        acc -s pc=45 rc=43 mcc=500 mcv=3920
            Dies hält die Batteriekapazität zwischen 43-45 %, begrenzt den Ladestrom auf 500 mA und die Spannung auf 3920 Millivolt.
...
print_quit() {
  echo "(i) Drücke $1 um abzubrechen/beenden"
  [ -z "${2-}" ] || echo "- Oder $2 um speichern und dann schließen"
}

print_curr_restored() {
    echo "(i) Voreingestellter maximaler Ladestrom wiederhergestellt"
}

print_volt_restored() {
    echo "(i) Voreingestellte maximale Ladespannung wiederhergestellt"
}

print_read_curr() {
    echo "(i) Der Standardwert für den maximalen Ladestrom muss zuerst gelesen werden"
}

print_curr_set() {
    echo "(i) Maximaler Ladestrom auf $1$(print_mA) gesetzt"
}

print_volt_set() {
    echo "(i) Maximale Ladespannung eingestellt auf $1$(print_mV)"
}
...
print_press_key() {
    printf "(i) Drücken Sie eine beliebige Taste, um fortzufahren..."
}
...
print_m_mode() {
    echo "(i) Manueller Modus"
}

print_wait() {
    echo "(i) Gut, das kann eine Minute oder so dauern..."
}
